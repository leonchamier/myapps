...
function sqlInsert(file,useobj,useconn,sqlphppgm) {
  ...
  //if (dbasetype=='access') {
  //    f=getDbTableAttr(file);
  //    if (f) {
  //        f=f.field.split(',');
  //        for (c=0; c<f.length; c++) {
  //             if (!useobj[f[c]]) {useobj[f[c]]=' '}
  //        }
  //    }
  //}
...
function massSqlInsert(tablename,sqlselectobj,connect,perbatch) {
  ...
  if (dbasetype != 'access') {
      //u$$string1 += " values"; for sql server above 2008
        u$$string1 += " select";
  }
  else {
  ...
  for (var i=0; i<sqlselectobj.rcdcnt; i++) {
       ...
       if (dbasetype != 'access') {
           //if (values) {values += ", ";} for sql server above 2008
           //values +='('+col$data+')';    for sql server above 2008
        ...
        if (batch==fullbatch) {
            ...
            if (runenvironment=='intranet') {
                 try {connect.Execute(outtxt);} 
                 catch (e) {sqlerr=e.message; return false}
            }
            else {
...
//       NOTE!! a field whose value is to be saved (in another field) for the multiple output rows generated, must be include as a field to split 
//
// example: wrapSqlSelectCol($s,'descr:20','marks:25','bilref:15:bilref2')
  ...
  eval('var obj2=new sqlSelectResult('+txt+')');
  var links=false;
...
function keySqlSuggest(fld,keyobj,keyfld,vlu) {
  ...
  for (p in keyobj) {
       if (wasblank || p.toLowerCase().substr(0,vlulen)==vlu) {
           txt += "<div class=suggestitem onmouseover=colorBg(this) onmouseout=colorBg(this,'*dft') onclick=changeVar(newfld,this.innerText)>"+keyobj[p][keyfld]+"</div>"; 
           reccnt++;
       }
       if (wasblank && reccnt==20) {break}
  }
...
function sqlLookUp($t) {
  ...
  if ($t.alwaysrefresh || $t.refresh==true || refreshdone==true || newrun==true || $t.sqlselect != $t.sqlprevselect) {
     ...
     if (prefix=='') {
        ...
        suffix += '<span id='+$t.lookupfld+'msg class=label style="width:240px;"></span>';
...
function getPcFilePath(pcfile) {
  ...
  try {var afile = ''+fso.GetFile(pcfile);} 
